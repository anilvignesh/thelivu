...
3. `PROJECT-STATUS.md` — living state; update it as you work.
4. `MANUAL.md` — every bot command, for the owner.
5. `docs/bio-page.md`, `docs/article-hosting.md` — design notes for the web
...
absolute — there is deliberately no bypass flag. Do not add one.**
...
2. **Approve** (`_handle_approve` in `thelivu_bot/bot.py`): prepares the draft
   (`publishing.parser.prepare_for_publish`), parses it, writes a `slug` onto
   the run, posts an HTML teaser to the channel linking
   `SLIDE_SERVER_BASE_URL/a/<slug>`, adds a **relative** `/a/<slug>` link to
   `bio_links` (the bio page), marks the run `published`, queues a carousel.
3. Orchestrator's next tick composes the carousel (Claude decides slide text
   and `DARK: true/false` — light kraft vs dark ink is **an intentional
   per-story mood choice**, owner approved keeping it), renders PNGs, sends the
   album back to the owner with Post/Kill buttons.
4. Owner approves → posted to Instagram via the Graph API (slide images are
   fetched by Meta from our own domain). Slide files are auto-deleted once a
   carousel reaches a terminal state (`cleanup_finished_carousels`).
...
8. Deploys take ~2–3 minutes; verify with the `deployment list` status plus a
   `curl` of the live surface before telling the owner something is live.
...
`PROJECT-STATUS.md`). Transparent perspective, English, Kerala-first tiers,
one reviewed piece a day (cadence promise deliberately NOT made in public
copy yet — validation week pending). Sources are leads; the open web
verifies. The publisher stage is deliberately dumb: **never run an LLM over
an approved draft**.
